...
/**
 * eUICC vendor detection and AID list transformation.
 * Directly based on EasyEUICC's Vendors.kt.
 */
...
interface EuiccVendor {
    ...
    /**
     * Some removable eSIM products prefer vendor-specific AID lists or ordering.
     * For example, multi-SE products from eSTK.me prefer SE0 and SE1 AIDs first.
     *
     * Returns a pair of (transformed AID list, decider for when to stop opening more channels).
     * If null, no transformation is needed.
     */
...
/**
 * eSTK.me vendor implementation
 */
class ESTKme : EuiccVendor {
    companion object {
        private const val TAG = "ESTKme"
        private val PRODUCT_AID = "A06573746B6D65FFFFFFFFFFFF6D6774".hexToByteArray()

        val ESTK_SE0_AID = "A06573746B6D65FFFF4953442D522030".hexToByteArray()
        val ESTK_SE1_AID = "A06573746B6D65FFFF4953442D522031".hexToByteArray()
    }
    ...
    override fun tryParseEuiccVendorInfo(channel: EuiccChannel): EuiccVendorInfo? {
        ...
        return try {
            withLogicalChannel(iface, PRODUCT_AID) { transmit ->
                fun invoke(p1: Byte) =
    ...
    override fun transformAidListIfNeeded(
        ...
        try {
            withLogicalChannel(referenceChannel.apduInterface, PRODUCT_AID) {}
        } catch (_: Exception) {
            // Not eSTK!
            return null
        }
        ...
        return if (expected == aidList) {
            ...
            Pair(expected, VendorAidDecider { openedAids, nextAid ->
                ...
                !(openedAids.isNotEmpty() && nextAid.contentEquals(Iso7816.EUICC_DEFAULT_ISDR_AID_HEX.hexToByteArray()))
            })
        }
    }
}
...
// Helper to open logical channel and close it after use
private inline fun <T> withLogicalChannel(
    apduInterface: net.typeblog.lpac_jni.ApduInterface,
    aid: ByteArray,
    block: (transmit: (ByteArray) -> ByteArray) -> T
): T {
    val handle = apduInterface.logicalChannelOpen(aid)
    try {
        return block { tx -> apduInterface.transmit(handle, tx) }
    } finally {
